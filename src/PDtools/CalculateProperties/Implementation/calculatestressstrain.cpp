...
void CalculateStressStrain::update3d() {
  ...
  for (int i = 0; i < nParticles; i++) {
    ...
    P(0, 0) = (lambda + 2 * mu) * strain(0, 0) + lambda * strain(1, 1) +
              lambda * strain(2, 2);
    P(1, 1) = lambda * strain(0, 0) + (lambda + 2 * mu) * strain(1, 1) +
              lambda * strain(2, 2);
    P(2, 2) = lambda * strain(0, 0) + lambda * strain(1, 1) +
              (lambda + 2 * mu) * strain(2, 2);
