...
PD_Particles convertMshToPdParticles(int dim, int interpolationDegree,
  ...
  // For the computation of the derivaties in the Jakobi determinant, not needed
  // for triangles
  vector<vector<double>> g1;
  vector<vector<double>> g2;

  for (int i = 0; i < interpolationDegree; i++) {
    vector<double> a1 = {-(1 - quad_points[i]), 1 - quad_points[i],
                         1 + quad_points[i], -(1 + quad_points[i])};
    vector<double> a2 = {-(1 - quad_points[i]), -(1 + quad_points[i]),
                         1 + quad_points[i], (1 - quad_points[i])};
    for (int j = 0; j < 4; j++) {
      a1[j] *= 1. / 4.;
      a2[j] *= 1. / 4.;
