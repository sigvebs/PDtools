...
PdMesh load_msh2d(string loadPath) {
  ...
  for (size_t i = 0; i < nVertices; i++) {
    ...
    boost::split(coordinateData, line, boost::is_any_of("\t "),
                 boost::token_compress_on);

    if (coordinateData.size() > 3) {
      verticeIds.push_back(stoi(coordinateData[0]));
      array<double, 3> coord = {stod(coordinateData[1]),
                                stod(coordinateData[2]),
                                stod(coordinateData[3])};
      vertices.push_back(coord);
    } else {
      cerr << "Error reading mesh from file, vertices elements not matching."
           << endl;
      cerr << loadPath << endl;
    }
  }
  ...
  if (!boost::iequals(line, "$EndNodes")) {
    cerr << "Error in reading mesh. Noes not ended correctly with $EndNodes"
         << endl;
  }
  ...
  for (size_t i = 0; i < nElements; i++) {
    ...
    boost::split(elementData, line, boost::is_any_of("\t "),
                 boost::token_compress_on);
    ...
    switch (elementType) {
    case TRIANGLE:
      element = {stoi(elementData[3 + nTags]), stoi(elementData[3 + nTags + 1]),
