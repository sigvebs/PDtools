...
void StrainFracture::evaluateStepOne(const int id_i, const int i)
{
    ...
    if(m_dim == 2)
    {
        for(auto &con:PDconnections)
        {
            ...
            const double ex = 0.5*(data(i, m_indexStrain[0]) + data(j, m_indexStrain[0]));
            const double ey = 0.5*(data(i, m_indexStrain[1]) + data(j, m_indexStrain[1]));
            const double exy = 0.5*(data(i, m_indexStrain[2]) + data(j, m_indexStrain[2]));

            const double Eeq = 2./9.*sqrt(pow(ex -ey,2) + ex*ex + ey*ey + 4./3.*exy*exy);
