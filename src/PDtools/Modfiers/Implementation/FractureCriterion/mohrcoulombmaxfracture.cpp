...
namespace PDtools {
//------------------------------------------------------------------------------
MohrCoulombMaxFracture::MohrCoulombMaxFracture(double mu, double C, double T)
    : m_C(C), m_T(T) {
  m_phi = mu * M_PI / 180.;
  m_d = tan(m_phi);
  m_neededProperties = {pair<string, int>("stress", 1),
                        pair<string, int>("damage", 1)};
...
void MohrCoulombMaxFracture::evaluateStepOne(const int id_i, const int i) {
  ...
  for (auto &con : PDconnections) {
    ...
    if (broken_j > 0) {
      ...
      const double radius_j = 1.5 * data(j, m_indexRadius);
  ...
  for (auto broken_idList : m_brokenParticles) {
    ...
    for (auto &con : PDconnections) {
      for (int id : broken) {
        if (id == id_i) {
          con.second[m_indexConnected] = 0;
          const int j = idToCol.at(id);
          (*m_data)(j, m_indexBrokenNow) = 1;
        }
      }
    }
  }
...
void MohrCoulombMaxFracture::evaluateStepTwo(const int id_i, const int i) {
  ...
  if (m_dim == 2) {
    ...
    double s_max = std::max(sx, sy);
    double s_min = std::min(sx, sy);
    double theta = 0.5 * std::atan(2. * sxy / (s_min - s_max));
    int broken = 0;
    const double criticalShear =
        std::fabs(shear) - std::fabs(m_C - m_d * normal);
    ...
    if (broken > 0) {
      ...
      cout << "# b: " << id_i << "\t t1:" << t1 * 180. / M_PI
           << " t2:" << t2 * 180. / M_PI << "|\t i:" << damageDiag_1
           << " j:" << damageDiag_2 << " t0:" << theta0 * 180. / M_PI
           << " frac:" << theta * 180. / M_PI << endl;
      //                 << "\t b:" << broken << endl;
      /*
      ...
      double ny_ = fx_;
      cout << "# b: " << id_i
           << "\t t1:" << t1*180./M_PI << " t2:" << t2*180./M_PI  << " tp:" <<
      theta*180./M_PI
      ...
      cout << "fx[0] = "     << r(i, 0) - radius*fx
           ...
           << endl;
           */
      //            cout << "# b: " << id_i << "\t theta:" << theta*180./M_PI <<
      //            " theta:" << theta0*180./M_PI <<  "\t x:" << nx << "\t y:"
      //            << ny << "\ti:" << i1 << " j:" << j1 << "\t b:" << broken <<
      //            endl;
      //            cout << "# sx:" << sx << "\tsy:" << sy << "\tsxy:" << sxy <<
      //            endl;
    }
  }
}
