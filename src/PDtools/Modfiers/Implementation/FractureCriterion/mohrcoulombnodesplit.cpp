...
namespace PDtools {
//------------------------------------------------------------------------------
MohrCoulombNodeSplit::MohrCoulombNodeSplit(double mu, double C, double T)
    : m_C(C), m_T(T) {
  m_phi = mu * M_PI / 180.;
  m_d = tan(m_phi);
  ...
  for (int i = 0; i < nParticles; i++) {
    ...
    if (m_dim == 2) {
      ...
      if (data(i, m_indexBroken) > 0 &&
          m_particles->pdConnections(id_i).size() <= 0) {
        data(i, m_indexBroken) = 0;
      }
      ...
      if (data(i, m_indexBroken) > 0) {
        ...
        double theta = 0.5 * std::atan(2. * sxy / (s_max - s_min));
        ...
        for (auto &con : PDconnections_i) {
          ...
          if (nId == id1)
            connectionsVector1.push_back(
                pair<int, vector<double>>(id_j, newCon));
          else
            connectionsVector2.push_back(
                pair<int, vector<double>>(id_j, newCon));
        }
    ...
    for (auto &con : PDconnections) {
      ...
      if (broken_j > 0) {
        ...
        for (auto &con2 : PDconnections2) {
          ...
          //                    cout << "iwdwe " << id_i << " " << con2.first
          //                    << endl;
          //                    cout << "break " << id_i << "(" << id_j << ") -
          //                    " << id_k << "\t " << px << "\t " << py << "\t
          //                    (" << x1 << ", " << y1 << ")" << endl;
        }
        //--------------------------------------------------------------
  ...
  for (int i = 0; i < nParticles; i++) {
    ...
    for (auto &con : PDconnections) {
      ...
      for (int id : m_toBeDeleted) {
        if (id == id_j) {
          cout << "id_i:" << id_i << " connected:" << id_j << " (broken)"
               << endl;
        }
      }
    }
  ...
  for (int i = 0; i < nParticles; i++) {
    ...
    for (auto &con_i : PDconnections) {
      ...
      vector<pair<int, vector<double>>> &PDconnections_j =
          m_particles->pdConnections(id_j);
      ...
      if (!found) {
        cout << "not found id_i:" << id_i << " in " << id_j << " connnection"
             << endl;
      }
    }
  }
...
void MohrCoulombNodeSplit::copyParticleTo(int id_to, int old_col, int new_col) {
  ...
  for(int i=0;i<nPdConnections; i++)
  {
      ...
      connectionsVector.push_back(pair<int, vector<double>>(con_id,
  connectionData));
  }
  // Verlet lists
