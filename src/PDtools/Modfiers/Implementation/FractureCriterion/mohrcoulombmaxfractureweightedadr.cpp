...
{
//------------------------------------------------------------------------------
MohrCoulombMaxFractureWeightedAdr::MohrCoulombMaxFractureWeightedAdr(double mu,
                                                                     double C,
                                                                     double T,
                                                                     double Wc,
                                                                     double Bc):
...
//------------------------------------------------------------------------------
void MohrCoulombMaxFractureWeightedAdr::evaluateStepOne(const int id_i, const int i)
{
...
//------------------------------------------------------------------------------
void MohrCoulombMaxFractureWeightedAdr::evaluateStepTwo(const int id_i, const int i)
{
    mat & data = *m_data;
    ...
    if(! broken_nodes.empty())
    {
        ...
        for(auto &con:PDconnections)
        {
            ...
            for(int id_g:broken_nodes)
            {
                ...
                double dr2 = (p[0] - r(g, 0))*(p[0] - r(g, 0)) + (p[1] - r(g, 1))*(p[1] - r(g, 1));
...
//                        ndr_j -= ij[d]*(r(g, d) - r(j, d));
                    ...
                    double sx, sy, sxy;
                    sx = 0.5*m_Bn*(data(i, m_indexStress[0]) + data(j, m_indexStress[0])) + m_Bc*data(g, m_indexStress[0]);
                    sy = 0.5*m_Bn*(data(i, m_indexStress[1]) + data(j, m_indexStress[1])) + m_Bc*data(g, m_indexStress[1]);
                    sxy = 0.5*m_Bn*(data(i, m_indexStress[2]) + data(j, m_indexStress[2])) + m_Bc*data(g, m_indexStress[2]);
