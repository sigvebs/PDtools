...
namespace PDtools {
//------------------------------------------------------------------------------
vector<int> Grid::nCpuGrid() const { return m_nCpuGrid; }
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
int Grid::dim() const { return m_dim; }
//------------------------------------------------------------------------------
void Grid::dim(int dim) { m_dim = dim; }
//------------------------------------------------------------------------------
Grid::Grid() {}
//------------------------------------------------------------------------------
Grid::Grid(const vector<pair<double, double>> &boundary, double gridspacing)
    : m_dim(boundary.size()), m_gridspacing(gridspacing), m_boundary(boundary),
...
void Grid::createGrid() {
  ...
  for (int d = 0; d < M_DIM; d++) {
    if (m_periodicBoundaries[d]) {
      inner_points.push_back(arma::linspace<ivec>(1, m_nGrid[d], m_nGrid[d]));
      ...
      inner_points.push_back(arma::linspace<ivec>(0, m_nGrid[d] - 1, m_nGrid[d]));
  ...
  for (int x : inner_points[X]) {
    for (int y : inner_points[Y]) {
      for (int z : inner_points[Z]) {
        // id = x + nx*y + nx*ny*z;
        // Center of gridpoint
        const vec3 center = {m_boundary[X].first + m_gridSpacing(X) * (x + 0.5),
                             m_boundary[Y].first + m_gridSpacing(Y) * (y + 0.5),
                             m_boundary[Z].first +
                                 m_gridSpacing(Z) * (z + 0.5)};
  ...
  for (int d = 0; d < M_DIM; d++) {
    ...
    for (int point_0 : boundary_points[d]) {
      ...
      const ivec p1 =
          join_cols(inner_points[inn_p[0]], boundary_points[inn_p[0]]);
      for (int point_1 : p1) {
        const ivec p2 =
            join_cols(inner_points[inn_p[1]], boundary_points[inn_p[1]]);
...
int Grid::gridIdN(const vector<int> &xyz) const {
  const vec3 center = {m_boundary[X].first + m_gridSpacing(X) * (xyz[X] + 0.5),
                       m_boundary[Y].first + m_gridSpacing(Y) * (xyz[Y] + 0.5),
                       m_boundary[Z].first + m_gridSpacing(Z) * (xyz[Z] + 0.5)};
...
//------------------------------------------------------------------------------
const std::vector<int> &Grid::ghostGrid() const { return m_ghostGridIds; }
//------------------------------------------------------------------------------
void Grid::setInitialPositionScaling(const double L0) { m_L0 = L0; }
//------------------------------------------------------------------------------
double Grid::initialPositionScaling() const { return m_L0; }
//------------------------------------------------------------------------------
const arma::ivec3 &Grid::nGrid() const { return m_nGridArma; }
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
void GridPoint::setnGridId(const vector<int> &ids) { m_nGridId = ids; }
//------------------------------------------------------------------------------
const vector<int> &GridPoint::nGridId() const { return m_nGridId; }
//------------------------------------------------------------------------------
vector<double> GridPoint::periodicShift() const { return m_periodicShift; }
//------------------------------------------------------------------------------
void GridPoint::setPeriodicShift(double shift, int d) {
  m_periodicShift[d] = shift;
}
//------------------------------------------------------------------------------
int GridPoint::periodicNeighbourRank() const { return m_periodicNeighbourRank; }
//------------------------------------------------------------------------------
void GridPoint::setPeriodicNeighbourRank(const int periodicNeighbourRank) {
  m_periodicNeighbourRank = periodicNeighbourRank;
}
//------------------------------------------------------------------------------
vector<array<size_t, 2>> GridPoint::elements() const { return m_elements; }
//------------------------------------------------------------------------------
// Other grid dependent functions
//------------------------------------------------------------------------------

void updateVerletList(const string &verletStringId, Particles &particles,
