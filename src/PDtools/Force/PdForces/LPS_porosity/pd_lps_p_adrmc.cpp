...
{
//------------------------------------------------------------------------------
PD_LPS_porosity_adrmc::PD_LPS_porosity_adrmc(PD_Particles &particles, double m, double b, double phi, double C, double T, bool planeStress, bool analyticalM):
    LPS_porosity_mc(particles, 0., m, b, phi, C, T, planeStress, analyticalM)
{
...
//            const int criticalTensile = s >= s_crit;
            ...
            if (MC_valid) {
                if(criticalShear) {
                    ...
                    broken = true;
//                    cout << "Shear\t " << id << " - " << id_j <<  "\tp1:" << p_1 << ", " << p_2 << endl;
                }
            }
//            } else {
//            }
            if(criticalTensile) {
                ...
                m_continueState = true;
                broken = true;
//                cout << "Tensile\t " << id << " - " << id_j <<  "\tp1:" << p_1 << ", " << p_2 << endl;
            }
        }
    } else if(m_dim == 3) {
