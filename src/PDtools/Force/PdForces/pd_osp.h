...
public:
  ...
  virtual void initialize(double E, double nu, double delta, int dim, double h,
                          double lc);
  virtual void applySurfaceCorrectionStep1(double mu, double nu, int dim,
                                           double strain);
  double calculateDilationTerm(const int id_i, const int i);
  double calculateBondPotential(const int id_i, const int i);
};
//------------------------------------------------------------------------------
}
#endif // PD_OSP_H
