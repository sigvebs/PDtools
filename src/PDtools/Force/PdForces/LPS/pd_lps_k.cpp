...
void PD_LPS_K::calculateForces(const int id, const int i) {
  ...
  if (m_dim == 3) {
    ...
    for (int l_j = 0; l_j < nConnections; l_j++) {
      ...
      const double Fx =
          dr_ij[0] * bond(0, 0) + dr_ij[1] * bond(0, 1) + dr_ij[2] * bond(0, 2);
      const double Fy =
          dr_ij[0] * bond(1, 0) + dr_ij[1] * bond(1, 1) + dr_ij[2] * bond(1, 2);
      const double Fz =
          dr_ij[0] * bond(2, 0) + dr_ij[1] * bond(2, 1) + dr_ij[2] * bond(2, 2);
      m_Fx[i] += Fx;
      m_Fy[i] += Fy;
      m_Fz[i] += Fz;
...
double PD_LPS_K::calculatePotentialEnergyDensity(const int id_i, const int i) {
  ...
  for (int l_j = 0; l_j < nConnections; l_j++) {
    ...
    const double extension_term =
        m_alpha * w * (pow(ds - theta_i * dr0 / m_dim, 2));
...
//------------------------------------------------------------------------------
void PD_LPS_K::calculatePotentialEnergy(const int id_i, const int i,
                                        int indexPotential) {
...
//------------------------------------------------------------------------------
void PD_LPS_K::initialize(double E, double nu, double delta, int dim, double h,
                          double lc) {
...
//------------------------------------------------------------------------------
void PD_LPS_K::calculateStress(const int id_i, const int i,
