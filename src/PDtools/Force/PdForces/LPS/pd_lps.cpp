...
void PD_LPS::calculateForces(const int id, const int i) {
  ...
  if (m_dim == 3) {
    ...
    for (int l_j = 0; l_j < nConnections; l_j++) {
      ...
      dr_ij[0] = m_x[j] - m_x[i];
      dr_ij[1] = m_y[j] - m_y[i];
      const double dr2 = dr_ij[0] * dr_ij[0] + dr_ij[1] * dr_ij[1];
...
//------------------------------------------------------------------------------
void PD_LPS::calculatePotentialEnergy(const int id_i, const int i,
                                      int indexPotential) {
...
//------------------------------------------------------------------------------
void PD_LPS::calculateStress(const int id_i, const int i,
