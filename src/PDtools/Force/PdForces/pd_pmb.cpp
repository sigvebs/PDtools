...
void PD_PMB::calculateForces(const int id_i, const int i) {
  ...
  s0_new = std::numeric_limits<double>::max();
  //    first = true;
  ...
  for (int jj = 0; jj < jnum; jj++) {
    ...
    if (stretch > std::min(s0_i, s0_j)) {
      if (m_data(i, m_indexUnbreakable) > 0 ||
          m_data(j, m_indexUnbreakable) > 0) {
...
double PD_PMB::calculatePotentialEnergyDensity(const int id_i, const int i) {
  ...
  for (auto &con : PDconnections) {
    ...
    const double drSquared =
        dr_ij[X] * dr_ij[X] + dr_ij[Y] * dr_ij[Y] + dr_ij[Z] * dr_ij[Z];
...
//------------------------------------------------------------------------------
void PD_PMB::calculatePotentialEnergy(const int id_i, const int i,
                                      int indexPotential) {
  const double vol_i = m_data(i, m_indexVolume);
  m_data(i, indexPotential) += calculatePotentialEnergyDensity(id_i, i) * vol_i;
}
//------------------------------------------------------------------------------
void PD_PMB::calculateStress(const int id_i, const int i,
