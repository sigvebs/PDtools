...
//------------------------------------------------------------------------------
SavePdData::SavePdData(std::vector<string> saveParameters) : SavePdData() {
...
void SavePdData::initialize() {
  ...
  for (string param : m_saveParameters) {
    if (param == "damage") {
      m_neededProperties.push_back(
          pair<string, int>("damage", m_updateFrquency));
      ...
      m_saveparam_scale.push_back(
          std::pair<std::string, double>("max_stretch", 1.));
    } else if (param == "radius") {
      m_saveparam_scale.push_back(
          std::pair<std::string, double>("radius", m_L0));
    } else if (param == "porosity") {
      m_saveparam_scale.push_back(
          std::pair<std::string, double>("porosity", 1.));
    } else if (param == "average_stretch") {
      m_computeProperties.push_back(new ComputeAverageStretch(*m_particles));
      m_saveparam_scale.push_back(
          std::pair<std::string, double>("average_stretch", 1.));
    } else if (param == "kinetic_energy") {
      m_computeProperties.push_back(new ComputeKineticEnergy(*m_particles));
      m_saveparam_scale.push_back(std::pair<std::string, double>(
          "kinetic_energy", pow(m_v0, 2) * pow(m_L0, 3) * m_rho0));
    } else if (param == "potential_energy") {
      m_computeProperties.push_back(
          new ComputePotentialEnergy(*m_particles, *m_oneBodyForces));
      m_saveparam_scale.push_back(
          std::pair<std::string, double>("potential_energy", 1.));
    } else if (param == "stress") {
      computeStress = true;
      m_neededProperties.push_back(
          pair<string, int>("stress", m_updateFrquency));
    } else if (param == "id") {
      ...
      m_neededProperties.push_back(
          pair<string, int>("strain", m_updateFrquency));
    } else {
...
void SavePdData::saveData(double t, int i) {
  (void)t;
  string baseName = m_savePath + "/" + std::to_string(i) + ".lmp";
...
//------------------------------------------------------------------------------
void SavePdData::setSavePath(string savePath) { m_savePath = savePath; }
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
void SavePdData::setScaling(const double E0, const double L0, const double v0,
...
//------------------------------------------------------------------------------
void SavePdData::setGrid(Grid *grid) { m_mainGrid = grid; }
//------------------------------------------------------------------------------
bool SavePdData::writeBinary() const { return m_writeBinary; }
//------------------------------------------------------------------------------
void SavePdData::setWriteBinary(bool writeBinary) {
  m_writeBinary = writeBinary;
}
//------------------------------------------------------------------------------
int SavePdData::updateFrquency() const { return m_updateFrquency; }
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
void SavePdData::setDim(int dim) { m_dim = dim; }
//------------------------------------------------------------------------------
