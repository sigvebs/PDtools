...
//    double gridspacing = 1.25*(delta + 0.5*lc);
    ...
    string particlesPath = static_cast<const char *>(m_cfg.lookup("particlesPath"));
    ...
    for(int i=0; i<cfg_forces.getLength(); i++) {
        ...
        if(boost::iequals(type, "bond force")) {
            ...
            forces.push_back(new PD_PMB_LINEAR_INTEGRATOR(m_particles, lc, delta, alpha));
        }
...
//            forces.push_back(new PD_LPS(m_particles, planeStress, analyticalM));
            ...
            forces.push_back(new PD_lpsDampenedContact(m_particles, c/t0, planeStress));
            ...
            if(boost::iequals(solverType, "ADR")) {
                forces.push_back(new PD_LPS_adrmc(m_particles, mu, C, T, planeStress, analyticalM));
            } else {
                forces.push_back(new LPS_mc(m_particles, c/t0, mu, C, T, planeStress, analyticalM));
            }
        }
            ...
            if(boost::iequals(solverType, "ADR")) {
                forces.push_back(new PD_LPS_ADR_STRAIN(m_particles, c/t0, stretchCrit, shearCrit, planeStress, analyticalM));
            } else {
                forces.push_back(new PD_LPS_CRIT_STRAIN(m_particles, c/t0, stretchCrit, shearCrit, planeStress, analyticalM));
            }
        } // -------- NOSBPD MODELS --------
        ...
        // -------- LPS POROSITY MODELS --------
            ...
            forces.push_back(new PD_LPS_POROSITY(m_particles, phi_c, n, planeStress, analyticalM));
            ...
            forces.push_back(new PD_lpsDampenedContact_porosity(m_particles, phi_c, n, c/t0, planeStress));
        }
                    ...
                    !cfg_forces[i].lookupValue("T", T) )
            {
                ...
                cerr << "The fracture parameters 'mu', 'C' and 'T' must be set for " << endl;
            ...
            if(boost::iequals(solverType, "ADR")) {
                forces.push_back(new PD_LPS_porosity_adrmc(m_particles, m, b, mu, C, T, planeStress, analyticalM));
            } else {
                forces.push_back(new LPS_porosity_mc(m_particles, m, b, c/t0, mu, C, T, planeStress, analyticalM));
            }
        }
...
//    MPI_Barrier(MPI_COMM_WORLD);
    ...
    for(int i=0; i<cfg_modifiers.getLength(); i++) {
        ...
        if(boost::iequals(type, "velocity Particles")) {
            ...
            double volume = (a1 - a0)*dxdydz[0]*dxdydz[1]*dxdydz[2]/dxdydz[axis];
            appliedForce /= volume;
            ...
            boundaryModifiers.push_back(new boundaryForce(appliedForce, forceAxis, area, axis, steps, delta, incremental, scaled));
...
//            vector<double> fracture = {0., 0.3, 0.09999, 0.100001}; // Test fracture
            vector<double> fracture = {0.,0.,0.,0.}; // Test fracture, [0, x1, y0, y1], all z
